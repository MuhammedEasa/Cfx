...
export const HeroSlider = () => {
  return (
    <div className="w-full max-w-[95%] sm:max-w-[100rem] mx-auto px-2 sm:px-4 lg:px-6 xl:px-8">
      <AnimationContainer animation="fadeUp" delay={0.2}>
        <div className="relative rounded-2xl lg:rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-r from-black/20 to-black/10 backdrop-blur-sm border border-white/10">
          ...
          >
            {SLIDER_DATA.map((slide) => (
              <SwiperSlide key={slide.id}>
                <div className="relative h-[450px] sm:h-[400px] lg:h-[500px] xl:h-[550px] w-full">
                  {/* Background Media */}
                  {slide.type === 'video' ? (
                    <video
                      src={slide.media}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-8 lg:px-16">
                    <div className="text-center text-white max-w-5xl w-full">
                      <div className={`inline-block px-4 py-2 sm:px-6 sm:py-3 rounded-full bg-gradient-to-r ${slide.accent} text-white text-xs sm:text-sm lg:text-base font-medium mb-4 sm:mb-6 shadow-lg`}>
                        {slide.subtitle}
                      </div>

                      <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl font-bold mb-4 sm:mb-6 lg:mb-8 leading-tight">
                        {slide.title}
                      </h2>

                      <p className="text-sm sm:text-base lg:text-xl xl:text-2xl mb-6 sm:mb-8 lg:mb-10 text-gray-200 max-w-3xl mx-auto leading-relaxed">
                        {slide.description}
                      </p>
                      ...
                      <Link href={slide.buttonLink}>
                        <Button
                          size="lg"
                          className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-3 sm:px-10 sm:py-4 text-sm sm:text-base lg:text-lg font-semibold rounded-full transition-all duration-300 hover:scale-105 shadow-xl border-2 border-primary/20"
                        >
                          {slide.buttonText}
                        </Button>
                      </Link>
          ...
          {/* Custom Navigation */}
          <button className="swiper-button-prev-custom absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300">
            <svg className="w-4 h-4 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>

          <button className="swiper-button-next-custom absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300">
            <svg className="w-4 h-4 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
